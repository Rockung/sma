...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (x, gamma, beta, sample_mean, sample_var, x_hat, eps)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    x, gamma, beta, sample_mean, sample_var, x_hat, eps = cache
    N = x.shape[0]
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        ...
        for j in range(W_out):
            j_s = j * stride
            x_mask = x_pad[:, :, i_s:i_s+HH, j_s:j_s+WW]
            for k in range(F):
                dw[k ,: ,: ,:] += np.sum(x_mask * (dout[:, k, i, j])[:, None, None, None], axis=0)
            for n in range(N):
                dx_pad[n, :, i_s:i_s+HH, j_s:j_s+WW] += np.sum((w[:, :, :, :] * (dout[n, :, i, j])[:,None ,None, None]), axis=0)
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(H2):
        for j in range(W2):
            ...
            temp_mask = (x_mask == (x_mask_max)[:, :, None, None])
            dx[:, :, i*S:i*S+PH, j*S: j*S+PW] += temp_mask * (dout[:, :, i, j])[:, :, None, None]
    ###########################################################################
    #                             END OF YOUR CODE          #
    ###########################################################################
    return dx
